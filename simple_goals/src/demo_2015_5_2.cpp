...
do
{
	goal.target_pose.pose.position.x = -2.430;
	goal.target_pose.pose.position.y = 0.096;
	goal.target_pose.pose.orientation = tf::createQuaternionMsgFromYaw(3.091);
...
{
	//中心点
	goal.target_pose.pose.position.x = 4.749;
	goal.target_pose.pose.position.y = -0.022;
	goal.target_pose.pose.orientation = tf::createQuaternionMsgFromYaw(-0.021);
...
{
	//格档点
	goal.target_pose.pose.position.x = 4.632;
	goal.target_pose.pose.position.y = -4.929;
	goal.target_pose.pose.orientation = tf::createQuaternionMsgFromYaw(-1.591);
...
{
	//厕所处
	goal.target_pose.pose.position.x = 11.196;
	goal.target_pose.pose.position.y = 0.701;
	goal.target_pose.pose.orientation = tf::createQuaternionMsgFromYaw(1.538);
...
{
	//起始点
	goal.target_pose.pose.position.x = 1.737;
	goal.target_pose.pose.position.y = 1.498;
	goal.target_pose.pose.orientation = tf::createQuaternionMsgFromYaw(3.103);
	ROS_INFO("Sending goal five");
	ac.sendGoal(goal);
	ac.waitForResult();
}while(ac.getState() != actionlib::SimpleClientGoalState::SUCCEEDED);
