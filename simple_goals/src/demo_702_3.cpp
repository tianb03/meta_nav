...
int main(int argc, char** argv)
{
  ...
  do
  {
    goal.target_pose.pose.position.x = -1.905;
    goal.target_pose.pose.position.y = -8.052;
    goal.target_pose.pose.orientation = tf::createQuaternionMsgFromYaw(1.153);
    ROS_INFO("Sending goal shiyanshi");
    ac.sendGoal(goal);
    ac.waitForResult();
